...
def _valid_result(path: str, name: str, plugin_type: str) -> dict:
    """Build a serialized-plugin dict the cache backend will accept on commit."""
    return {
...
def mock_plugin_discovery():
    ...
    with patch.object(
        IsolatedPedalboardScanner, "_find_plugins_to_scan", mock_find_plugins
    ):
        yield test_plugins
...
class TestWorkerProcessCrash:
    ...
    def test_worker_crash_simulation(self, test_cache_dir, mock_plugin_discovery):
        """A crashing worker is swallowed; the other plugins still commit."""
        with patch(
            "pedalboard_pluginary.scanner_isolated.get_cache_path",
            side_effect=lambda name: test_cache_dir / name,
        ):
            scanner = IsolatedPedalboardScanner(max_workers=1, timeout=5)
            journal_path = scanner.journal_path

            def mock_run_scan_single_crash(
                plugin_path,
                plugin_name,
                plugin_type,
                journal_path_str,
                ...
                if plugin_path == "/test/plugin2.vst3":
                    # Simulate a worker crashing before it can journal anything.
                ...
                journal.update_status(
                    plugin_path,
                    "success",
                    _valid_result(plugin_path, plugin_name, plugin_type),
                )
                journal.close()

            with patch(
                "pedalboard_pluginary.scanner_isolated.run_scan_single",
                side_effect=mock_run_scan_single_crash,
            ):
                # The crash must not abort the whole scan.
                scanner.scan()

            # The two non-crashed plugins are committed to the cache.
            committed = {p["path"] for p in scanner.cache_backend.get_all_plugins()}
            assert committed == {"/test/plugin1.vst3", "/test/plugin3.vst3"}

            # Journal is cleaned up once the successful results are committed.
    ...
    def test_worker_timeout(self, test_cache_dir, mock_plugin_discovery):
        ...
        with patch("pedalboard_pluginary.data.get_cache_path") as mock_get_cache:
            ...
            def mock_run_scan_single_timeout(
                plugin_path,
                plugin_name,
                plugin_type,
                journal_path_str,
...
class TestMainProcessCrash:
    ...
    def test_main_process_crash_and_resume(self, test_cache_dir, mock_plugin_discovery):
        ...
        with patch("pedalboard_pluginary.data.get_cache_path") as mock_get_cache:
            ...
            def mock_run_scan_single_partial(
                plugin_path,
                plugin_name,
                plugin_type,
                journal_path_str,
            ...
            def mock_run_scan_single_resume(
                plugin_path,
                plugin_name,
                plugin_type,
                journal_path_str,
...
class TestCommitPhaseCrash:
    ...
    def test_crash_during_commit(self, test_cache_dir, mock_plugin_discovery):
        ...
        with patch("pedalboard_pluginary.data.get_cache_path") as mock_get_cache:
            ...
            def mock_run_scan_single_success(
                plugin_path,
                plugin_name,
                plugin_type,
                journal_path_str,
            ...
            mock_cache.add_plugins.side_effect = RuntimeError(
...
class TestEdgeCases:
    ...
    def test_all_failed_journal(self, test_cache_dir, mock_plugin_discovery):
        ...
        with patch("pedalboard_pluginary.data.get_cache_path") as mock_get_cache:
            ...
            def mock_run_scan_single_fail(
                ...
                journal = ScanJournal(Path(journal_path_str))
                journal.update_status(
                    plugin_path, "failed", {"error": f"Failed to load {plugin_name}"}
                )
                journal.close()
    ...
    def test_mixed_status_journal(self, test_cache_dir, mock_plugin_discovery):
        ...
        with patch(
            ...
            def mock_run_scan_single_mixed(
                plugin_path,
                plugin_name,
                plugin_type,
                journal_path_str,
                ...
                if "plugin1" in plugin_path:
                    ...
                    journal.update_status(
                        plugin_path, "failed", {"error": "Load error"}
                    )
                else:
            ...
            # Verify only the successful plugin was committed
    ...
    def test_concurrent_journal_access(self, test_cache_dir):
        ...
        def worker_thread(thread_id: int, plugin_ids: list):
            ...
            for plugin_id in plugin_ids:
                ...
                journal.update_status(
                    plugin_id, "success", {"thread": thread_id, "plugin": plugin_id}
                )
            journal.close()
...
class TestRealProcessCrash:
    ...
    @pytest.mark.skipif(
        sys.platform == "win32", reason="Process signals differ on Windows"
    )
