...
def create_test_plugin(
    plugin_id: str, name: str, manufacturer: str = "TestMfg"
) -> PluginInfo:
    ...
    return PluginInfo(
        ...
        parameters={
            f"param_{i}": PluginParameter(name=f"param_{i}", value=float(i))
...
class TestSQLitePerformance:
    ...
    def test_sqlite_search_performance(self, sqlite_backend, medium_dataset):
        ...
        assert search_time < 0.1, (
            f"SQLite search took {search_time:.3f}s, expected < 0.1s"
        )
    ...
    def test_sqlite_filter_performance(self, sqlite_backend, medium_dataset):
        ...
        assert filter_time < 0.1, (
            f"SQLite filter took {filter_time:.3f}s, expected < 0.1s"
        )
    ...
    def test_sqlite_update_performance(self, sqlite_backend, small_dataset):
        ...
        assert update_time < 0.1, (
            f"SQLite update took {update_time:.3f}s, expected < 0.1s"
        )
    ...
    def test_memory_efficiency(self, sqlite_backend, large_dataset):
        ...
        assert load_memory_increase < 30, (
            f"Load memory increase {load_memory_increase:.1f}MB too high"
        )
