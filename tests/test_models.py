...
class TestPluginParameter:
    ...
    def test_init_with_various_value_types(self):
        """Test PluginParameter accepts the full range of ParameterValue types."""
        int_param = PluginParameter(name="Voices", value=8)
        str_param = PluginParameter(name="Mode", value="stereo")
        bool_param = PluginParameter(name="Bypass", value=True)

        assert int_param.value == 8
        assert str_param.value == "stereo"
