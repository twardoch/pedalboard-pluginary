...
class TestPluginaryExceptions:
    ...
    def test_pluginary_error_with_details(self):
        """Test PluginaryError with optional details."""
        error = PluginaryError("Test error", "Original error")
        assert error.message == "Test error"
        assert error.details == "Original error"
        assert str(error) == "Test error - Original error"
    ...
    def test_plugin_load_error_inheritance(self):
        ...
        error = PluginLoadError("/test/path/plugin.vst3")
        assert isinstance(error, ScannerError)
        assert isinstance(error, PluginaryError)
        assert isinstance(error, Exception)
        assert error.plugin_path == "/test/path/plugin.vst3"
        assert str(error) == "Failed to load plugin: /test/path/plugin.vst3"
    ...
    def test_exception_chaining(self):
        ...
        try:
            try:
                ...
                raise CacheError("Cache file not found") from e
