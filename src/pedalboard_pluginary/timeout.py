...
def sync_timeout(
    func: Callable[..., T], timeout: float, *args: Any, **kwargs: Any
) -> T:
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=1) as executor:
        ...
        try:
            ...
            future.cancel()
            raise TimeoutError(
                f"{func.__name__} timed out after {timeout}s", timeout
            ) from e
...
async def async_timeout(
    coro_func: Callable[..., Awaitable[T]], timeout: float, *args: Any, **kwargs: Any
) -> T:
    ...
    try:
        ...
        raise TimeoutError(
            f"{coro_func.__name__} timed out after {timeout}s", timeout
        ) from e
    except Exception as e:
