# this_file: src/pedalboard_pluginary/core.py
...
class PedalboardPluginary:
    ...
    def list_plugins(self, **filters: Any) -> list[dict]:
        ...
        plugin_list = []
        for _plugin_id, plugin in all_plugins.items():
            plugin_dict = {
                "id": plugin.id,
                "name": plugin.name,
                "path": plugin.path,
                ...
                "params": {k: v.value for k, v in plugin.parameters.items()}
            ...
            if filters:
                ...
                if "manufacturer" in filters and filters["manufacturer"]:
                    if (
                        not plugin.manufacturer
                        or filters["manufacturer"].lower()
                        not in plugin.manufacturer.lower()
                    ):
                        continue
